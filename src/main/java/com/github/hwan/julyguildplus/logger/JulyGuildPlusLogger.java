package com.github.hwan.julyguildplus.logger;
...
public class JulyGuildPlusLogger {
	public static void init() {
		File logFolder = new File(JulyGuildPlus.inst().getDataFolder(), "logs");
	...
	public static void setLevel(@NotNull LoggerLevel level) {
		JulyGuildPlusLogger.level = level;
	}
	...
	private static void log(@NotNull LoggerLevel loggerLevel, @NotNull String msg) {
		if (loggerLevel.getLevel() >= JulyGuildPlusLogger.level.getLevel()) {
	...
	private static void setNewWriter(@NotNull String fileName) {
		File file = new File(JulyGuildPlus.inst().getDataFolder(), "logs" + File.separator + fileName + ".log");
