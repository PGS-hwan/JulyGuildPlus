...
public abstract class BaseRequest<T1 extends Sender, T2 extends Receiver> implements Request {
    ...
    public void onLoad(@NotNull ConfigurationSection section) {
        ...
        switch (Receiver.Type.valueOf(section.getString("receiver.type"))) {
            case GUILD:
                this.receiver = (T2) JulyGuildPlus.inst().getGuildManager().getGuild(UUID.fromString(section.getString("receiver.guild.uuid")));
                break;
            case GUILD_PLAYER:
                this.receiver = (T2) JulyGuildPlus.inst().getGuildPlayerManager().getGuildPlayer(UUID.fromString(section.getString("receiver.guild_player.uuid")));
                break;
            case GUILD_MEMBER:
