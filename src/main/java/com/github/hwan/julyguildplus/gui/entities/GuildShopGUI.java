...
import com.github.hwan.julyguildplus.gui.ShopItemConfirmGUI;
import com.github.hwan.julyguildplus.guild.*;
...
public class GuildShopGUI extends BasePlayerGUI {
    ...
    protected GuildShopGUI(@Nullable GUI lastGUI, @NotNull GuildMember guildMember) {
        this(lastGUI, guildMember, Optional.ofNullable(JulyGuildPlus.inst().getShop(MainSettings.getGuildShopLauncher())).orElseThrow(() -> new RuntimeException("引导商店不存在")));
    }
    ...
    public Inventory createInventory() {
        ...
        JulyGuildPlusLogger.debug("尝试载入商店 " + shopName + ".");
        if (yml.contains("items")) {
            for (String shopItemName : yml.getConfigurationSection("items").getKeys(false)) {
                ...
                JulyGuildPlusLogger.debug("尝试获取物品类型 '" + shopItemSection.getCurrentPath() + "'.");
                RewardType rewardType = RewardType.valueOf(rewardTypeStr);
                JulyGuildPlusLogger.debug("获取物品类型完毕 '" + shopItemSection.getCurrentPath() + "'.");
        ...
        JulyGuildPlusLogger.debug("载入商店 " + shopName + " 完毕.");
    ...
    private void setCommandReward(@NotNull ConfigurationSection shopItemSection, @NotNull IndexConfigGUI.Builder guiBuilder) {
        JulyGuildPlusLogger.debug(DebugMessage.BEGIN_GUI_LOAD_ITEM, shopItemSection.getCurrentPath());
        ...
        JulyGuildPlusLogger.debug(DebugMessage.END_GUI_LOAD_ITEM, shopItemSection.getCurrentPath());
    }
                                ...
                                if (JulyMessage.isTitleEnabled()) {
                                    if (NMSUtil.compareVersion("v1_17_R1") >= 0) {
                                        receiverBukkitPlayer.sendTitle(
                                                com.github.hwan.julylib.text.JulyText.getColoredText(tpAllLangSection.getString("received.title"))
                                                , com.github.hwan.julylib.text.JulyText.getColoredText(tpAllLangSection.getString("received.subtitle"))
                                                , 0
                                                , 20
                                                , 20
                                        );
    ...
    private void setGuildUpgradeReward(@NotNull ConfigurationSection shopItemSection, @NotNull IndexConfigGUI.Builder guiBuilder) {
        ...
        if (available) {
            ...
            JulyGuildPlusLogger.debug(DebugMessage.BEGIN_GUI_LOAD_ITEM, subItemSection.getCurrentPath());
            ...
            JulyGuildPlusLogger.debug(DebugMessage.END_GUI_LOAD_ITEM, subItemSection.getCurrentPath());
        } else {
            JulyGuildPlusLogger.debug(DebugMessage.BEGIN_GUI_LOAD_ITEM, subItemSection.getCurrentPath());
