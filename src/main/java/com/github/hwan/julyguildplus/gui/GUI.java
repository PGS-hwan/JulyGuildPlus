...
public interface GUI {
    ...
    default void openLater(long tick) {
        ...
        }.runTaskLater(JulyGuildPlus.inst(), tick);
    }
    ...
    default void open() {
        ...
        JulyGuildPlusLogger.debug("=== 尝试创建GUI " + className + " ===");
        ...
        JulyGuildPlusLogger.debug("=== 创建GUI " + className + " 完毕 ===");

        getGuildPlayer().getBukkitPlayer().openInventory(inventory);
        getGuildPlayer().setUsingGUI(this);

        JulyGuildPlusLogger.debug("玩家 '" + getGuildPlayer().getName() + "' 打开了 GUI '" + className + "'.");
    }
    ...
    default void back(long later) {
        ...
        }.runTaskLater(JulyGuildPlus.inst(), later);
    }
